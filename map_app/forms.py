...
class ProjectForm(forms.ModelForm):
    ...
    additional_category = forms.ModelChoiceField(queryset=Category.objects.all(), required=False,
    ...
    class Meta:
        model = Project
        fields = ['title', 'description', 'latitude', 'longitude', 'category', 'main_type', 'additional_category',
                  'additional_type']
...
PhotoFormSet = modelformset_factory(Photo, fields=('image', 'description', 'is_main'), extra=1, can_delete=True)
VideoFormSet = modelformset_factory(Video, fields=('video', 'description'), extra=1, can_delete=True)
LinkFormSet = modelformset_factory(Link, fields=('url', 'description'), extra=1, can_delete=True)
